...
static std::string guid_string(const std::byte* p) {
    char b[37];
    auto b2 = [](unsigned x)->char { static const char* h="0123456789abcdef"; return h[x&15]; };
    unsigned d1=u32(p), d2=u16(p+4), d3=u16(p+6);
    std::snprintf(b,sizeof(b),"%08x-%04x-%04x-%c%c%c%c-%c%c%c%c%c%c",
                  d1,d2,d3,b2(std::to_integer<unsigned char>(p[8])>>4),b2(std::to_integer<unsigned char>(p[8])),
                  b2(std::to_integer<unsigned char>(p[9])>>4),b2(std::to_integer<unsigned char>(p[9])),
                  b2(std::to_integer<unsigned char>(p[10])>>4),b2(std::to_integer<unsigned char>(p[10])),
                  b2(std::to_integer<unsigned char>(p[11])>>4),b2(std::to_integer<unsigned char>(p[11])),
                  b2(std::to_integer<unsigned char>(p[12])>>4),b2(std::to_integer<unsigned char>(p[12])),
                  b2(std::to_integer<unsigned char>(p[13])>>4),b2(std::to_integer<unsigned char>(p[13])));
    return b;
}
