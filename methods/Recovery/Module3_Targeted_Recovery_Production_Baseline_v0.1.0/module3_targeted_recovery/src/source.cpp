...
public:
 ...
 bool read_at(std::uint64_t o,std::span<std::byte>b)override{
  if(o>n_||b.size()>n_-o||b.empty())return b.empty();
  LARGE_INTEGER li{};li.QuadPart=static_cast<LONGLONG>(o);if(!SetFilePointerEx(h_,li,nullptr,FILE_BEGIN))return false;
  constexpr std::size_t max_chunk=static_cast<std::size_t>(std::numeric_limits<DWORD>::max());
  std::size_t done=0;
  while(done<b.size()){
   const DWORD request=static_cast<DWORD>(std::min<std::size_t>(b.size()-done,max_chunk));
   DWORD got=0;
